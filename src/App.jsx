...
// garante yyyy-mm-dd para input type="date"
  ...
  function App() {
    ...
    const [deletedClientsMeta, setDeletedClientsMeta] = useState({
      page: 1,
      limit: 10,
      total: 0,
      has_next: false,
    ...
    const [deletedProductsMeta, setDeletedProductsMeta] = useState({
      page: 1,
      limit: 10,
      total: 0,
      has_next: false,
    ...
    const [deletedOrdersMeta, setDeletedOrdersMeta] = useState({
      page: 1,
      limit: 10,
      total: 0,
      has_next: false,
    ...
    const groupedOrders = useMemo(() => {
      ...
      const groupByStatus = (arr) => {
        const groups = {};
        STATUS_ORDER.forEach((groupStatus) => {
          groups[groupStatus] = [];
        });
        ...
        if ("groups" in group) {
          ...
          for (const subgroup of group.groups) {
            ...
            if (take > 0) {
              newSubgroups.push({
                ...subgroup,
                items: subgroupItems.slice(start, start + take),
              });
          ...
          if (take > 0) {
            result.push({
              ...group,
              items: groupItems.slice(start, start + take),
            });
            remainingToTake -= take;
          }
    ...
    async function removeProduct(productId) {
      ...
      try {
        ...
        await Promise.all([loadProducts(), loadAllProducts(), loadDeletedProducts()]);
      } catch (err) {
        console.error(err);
    ...
    async function loadDeletedProducts() {
      try {
        const res = await ordersApi.listDeletedProducts({
    ...
    async function loadDeletedClients() {
      try {
        const res = await ordersApi.listDeletedClients({
    ...
    async function loadDeletedOrders() {
      try {
        const res = await ordersApi.listDeletedOrders({
    ...
    async function saveClientEdit() {
      ...
      try {
        ...
        await Promise.all([loadClients(), loadAllClients()]);
        cancelEditClient();
      } catch (err) {
        console.error(err);
    ...
    async function removeClient(clientId) {
      ...
      try {
        ...
        await Promise.all([loadClients(), loadAllClients(), loadDeletedClients()]);
      } catch (err) {
        console.error(err);
    ...
    async function bulkUpdateStatus(toStatus) {
      ...
      const res = await ordersApi.updateMany({
        filter,
    ...
    async function bulkIncrement(fieldPath, value) {
      ...
      const res = await ordersApi.increment({
        filter,
    ...
    // A partir daqui entra a parte do HTML / JSX
    if (!authOk) {
    return (
              ...
              >
                ...
                <div className="row" style={{ gap: 8, alignItems: "center", marginLeft: "auto" }}>
                  <button
                    type="button"
                    className="btn"
                    disabled={deletedOrdersPage <= 1 || loading}
                    onClick={() => setDeletedOrdersPage((p) => p - 1)}
                  >
                    ◀ Anterior
                  </button>

                  <div className="mini">
                    Página: <strong>{deletedOrdersPage}</strong> • Total:{" "}
                    <strong>{deletedOrdersMeta.total}</strong>
                  </div>

                  <button
                    type="button"
                    className="btn"
                    disabled={!deletedOrdersMeta.has_next || loading}
                    onClick={() => setDeletedOrdersPage((p) => p + 1)}
                  >
                    Próxima ▶
                  </button>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setDeletedOrdersPage(1);
                      setDeletedOrdersLimit(Number(e.target.value));
                    }}
                    style={{ width: 140 }}
                  >
                    <option value={5}>5 / pág</option>
                  ...
                  {deletedOrders.map((order) => {
                    ...
                    return (
                      <div key={order._id} className="card orderCard" style={{ opacity: 0.9 }}>
                        ...
                        <div className="mini" style={{ marginTop: 8 }}>
                          Excluído em: {formatDateBR(order.deleted_at)}
                        </div>
              ...
              >
                ...
                <div className="row" style={{ gap: 8, alignItems: "center", marginLeft: "auto" }}>
                  <button
                    type="button"
                    className="btn"
                    disabled={deletedClientsPage <= 1 || loading}
                    onClick={() => setDeletedClientsPage((p) => p - 1)}
                  >
                    ◀ Anterior
                  </button>

                  <div className="mini">
                    Página: <strong>{deletedClientsPage}</strong> • Total:{" "}
                    <strong>{deletedClientsMeta.total}</strong>
                  </div>

                  <button
                    type="button"
                    className="btn"
                    disabled={!deletedClientsMeta.has_next || loading}
                    onClick={() => setDeletedClientsPage((p) => p + 1)}
                  >
                    Próxima ▶
                  </button>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setDeletedClientsPage(1);
                      setDeletedClientsLimit(Number(e.target.value));
                    }}
                    style={{ width: 140 }}
                  >
                    <option value={5}>5 / pág</option>
              ...
              >
                ...
                <div className="row" style={{ gap: 8, alignItems: "center", marginLeft: "auto" }}>
                  <button
                    type="button"
                    className="btn"
                    disabled={deletedProductsPage <= 1 || loading}
                  ...
                  <div className="mini">
                    Página: <strong>{deletedProductsPage}</strong> • Total:{" "}
                    <strong>{deletedProductsMeta.total}</strong>
                  </div>

                  <button
                    type="button"
                    className="btn"
                    disabled={!deletedProductsMeta.has_next || loading}
                    onClick={() => setDeletedProductsPage((p) => p + 1)}
                  >
                    Próxima ▶
                  </button>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setDeletedProductsPage(1);
                      setDeletedProductsLimit(Number(e.target.value));
                    }}
                    style={{ width: 140 }}
                  >
                    <option value={5}>5 / pág</option>
